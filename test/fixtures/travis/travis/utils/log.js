define('travis/utils/log', ['exports', 'npm:ansiparse'], function (exports, _npmAnsiparse) {
  'use strict';
  ...
  var newLineAtTheEndRegexp = void 0,
      newLineRegexp = void 0,
      rRegexp = void 0,
      removeCarriageReturns = void 0;
  ...
  Log.extend(Log.Part, {
    create: function create(log, num, string) {
      var part = void 0;
  ...
  Log.Part.prototype = Log.extend(new Log.Node(), {
    remove: function remove() {},
    process: function process(slice, num) {
      var _this = this;
  ...
  removeCarriageReturns = function removeCarriageReturns(string) {
    var index = void 0;
  ...
  Log.extend(Log.Span, {
    create: function create(parent, id, num, text, classes) {
      var span = void 0;
      span = new Log.Span(id, num, text, classes);
      parent.addChild(span);
      return span;
    },
    render: function render(parent, id, num, text, classes) {
      var span = void 0;
  ...
  Log.Span.prototype = Log.extend(new Log.Node(), {
    render: function render() {
      ...
      if (this.time && this.event === 'end' && this.prev) {
        if (Log.DEBUG) {
          var msg = 'S.0 insert ' + this.id + ' after prev ' + this.prev.id;
          console.log(msg);
        }
        ...
        if (Log.DEBUG) {
          var _msg = 'S.1 insert ' + this.id + ' after prev ' + this.prev.id;
          console.log(_msg);
        }
        this.log.insert(this.data, {
          after: this.prev.element
        ...
        if (Log.DEBUG) {
          var _msg2 = 'S.2 insert ' + this.id + ' before next ' + this.next.id;
          console.log(_msg2);
        }
        this.log.insert(this.data, {
  ...
  Log.extend(Log.Line, {
    create: function create(log, spans) {
      var line = void 0,
          span = void 0,
          _i = void 0,
          _len = void 0;
    ...
    render: function render() {
      ...
      if ((fold = this.prev) && fold.event === 'start' && fold.active) {
        if (this.next && !this.next.fold) {
          if (Log.DEBUG) {
            var msg = 'L.0 insert ' + this.id + ' before next ' + this.next.id;
            console.log(msg);
          }
          ...
          if (Log.DEBUG) {
            var _msg3 = 'L.0 insert ' + this.id + ' into fold ' + fold.id;
            console.log(_msg3);
          }
        ...
        if (Log.DEBUG) {
          var _msg4 = 'L.1 insert ' + this.spans[0].id + ' after prev ' + this.prev.id;
          console.log(_msg4);
        }
        return this.element = this.log.insert(this.data, {
          after: this.prev.element
        ...
        if (Log.DEBUG) {
          var _msg5 = 'L.2 insert ' + this.spans[0].id + ' before next ' + this.next.id;
          console.log(_msg5);
        }
        return this.element = this.log.insert(this.data, {
          before: this.next.element
        ...
        if (Log.DEBUG) {
          var _msg6 = 'L.3 insert ' + this.spans[0].id + ' into #log';
          console.log(_msg6);
        }
        return this.element = this.log.insert(this.data);
      }
  ...
  Log.Fold.prototype = Log.extend(new Log.Line(), {
    render: function render() {
      ...
      if (this.prev && this.prev.element) {
        if (Log.DEBUG) {
          var msg = 'F.1 insert ' + this.id + ' after prev ' + this.prev.id;
          console.log(msg);
        }
        ...
        if (Log.DEBUG) {
          var _msg7 = 'F.2 insert ' + this.id + ' before next ' + this.next.id;
          console.log(_msg7);
        }
  ...
  Log.extend(Log.Listener.prototype, {
    notify: function notify(log, event) {
      if (this[event]) {
  ...
  Log.extend(Log.Folds.Fold.prototype, {
    receive: function receive(data, options) {
    ...
    activate: function activate(options) {
      ...
      options || (options = {});
      if (Log.DEBUG) {
        var msg = 'F.n - activate ' + this.start;
        console.log(msg);
      }
      if (!this.fold) {
        return;
    ...
    classes: function classes(autoCloseFold) {
      var classes = void 0;
  ...
  Log.extend(Log.Times.Time.prototype, {
    receive: function receive(node) {
      this[node.event] = node;
      if (Log.DEBUG) {
        var msg = 'T.0 - ' + node.event + ' ' + node.name;
        console.log(msg);
      }
    ...
    finish: function finish() {
      var element = void 0;
      if (Log.DEBUG) {
        var msg = 'T.1 - finish ' + this.start.name;
        console.log(msg);
      }
  ...
  Log.Deansi = {
    ...
    apply: function apply(string) {
      ...
      return (0, _npmAnsiparse.default)(string).map(function (part) {
        return _this2.node(part);
      });
    },
    node: function node(part) {
      var classes = void 0,
          node = void 0;
      node = {
        type: 'span',
  ...
  Log.extend(Log.Renderer.prototype, {
    ...
    renderParagraph: function renderParagraph(data) {
      var node = void 0,
          para = void 0,
          type = void 0,
          _i = void 0,
    ...
    renderFold: function renderFold(data) {
      var fold = void 0;
    ...
    renderSpan: function renderSpan(data) {
      var span = void 0;
    ...
    createFold: function createFold() {
      var fold = void 0;
    ...
    insertBefore: function insertBefore(node, other) {
      var log = void 0;
      if (other) {
    ...
    insertAfter: function insertAfter(node, other) {
    ...
    prependTo: function prependTo(node, other) {
    ...
    appendTo: function appendTo(node, other) {
      if (other) {
        return other.appendChild(node);
      }
    },
    addClass: function addClass(classes, string) {

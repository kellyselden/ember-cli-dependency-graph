define('code-corps-ember/components/signup-username-input', ['exports'], function (exports) {
  'use strict';
  ...
  exports.default = Component.extend({
    ...
    checkAvailable: function checkAvailable() {
      ...
      this.sendRequest(username).then(function (result) {
        var available = result.available,
            valid = result.valid;

        var validation = valid && available;

        _this.set('cachedUsername', _this.get('username'));
        _this.set('hasCheckedOnce', true);
        _this.set('isChecking', false);
        _this.set('isAvailableOnServer', available);
        _this.set('isValid', valid);

        _this.set('canSubmit', validation);
        _this.sendAction('usernameValidated', validation);
      });
    },
